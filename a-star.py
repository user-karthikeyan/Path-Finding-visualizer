...
def find_path(start, end, grid, m, n):
    ...
    while heap:
        f_score, (x, y) = heappop(heap)
        current_g_score = g_score[x, y]
        ...
        for i, j in neighbours:
            ...
            if grid[new_x][new_y] != VISITED:
                if g_score[new_x, new_y] > current_g_score + 1:
                    color(grid, INQUEUE, new_x, new_y)
                    parent[new_x, new_y] = (x, y)
                    g_score[new_x, new_y] = current_g_score + 1
                    heappush(heap, (current_g_score + 1 + h_score(new_x, new_y), (new_x, new_y)))
